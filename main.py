...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--ffcv-load', default='Y', type=str, help='activate FFCV data loader [Y/N] default: Y')
    ...
    parser.add_argument('--eval-freq', help='perform evaluation every X epochs', default=None, type=int)
...
def main(args):
    logger = utils.CustomLogger(args)
    utils.init_distributed_mode(args)
    if args.output_dir and not os.path.exists(SAVED_MODELS_DIR):
        os.mkdir(SAVED_MODELS_DIR)
    # print(os.listdir('/mnt'))
    # print(os.listdir('/mnt/ImageNet'))
    # print(args)
    ...
    logger.log(f"{logger.now()} - Creating train dataset")
    if args.ffcv_load == 'Y':
        data_loader_train, args.nb_classes = ffcv_loader.create_train_loader(args)
    else:
        dataset_train, args.nb_classes = build_dataset(is_train=True, args=args)
    logger.log(f"{logger.now()} - Creating val dataset")
    if args.ffcv_load == 'Y':
        data_loader_val, _ = ffcv_loader.create_val_loader(args)
    else:
    ...
    logger.save_num_batches(data_loader_train, data_loader_val, args.batch_size)
    mixup_fn = None
    ...
    if args.distillation_type != 'none':
        ...
        logger.log(f"{logger.now()} - Creating teacher model: {args.teacher_model}")
    ...
    logger.log(f"{logger.now()} - Start training for {args.epochs} epochs")
    # logger.logs['init']['f_timestamp'] = logger.now()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.eval_freq and (epoch + 1) % args.eval_freq == 0:
            for data, t in zip([data_loader_train, data_loader_val], ['train', 'val']):
                logger.start_eval(epoch, t)
                evaluate(data, model, device, t, logger)
                logger.sum_eval(epoch, t)
            # test_stats = evaluate(data_loader_val, model, device)
            # print(
            #     f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        ...
        #                  'epoch': epoch,
        #                  'n_parameters': n_parameters}
        logger.sum_epoch(epoch)
        if args.output_dir and utils.is_main_process():
            # with (output_dir / "log.txt").open("a") as f:
            #     f.write(json.dumps(log_stats) + "\n")
            logger.save_logged_data()
